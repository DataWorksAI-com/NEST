...
ANTHROPIC_API_KEY="$1"
AGENT_CONFIG_JSON="$2"
REGISTRY_URL="${3:-http://23.92.20.191:6900}"
REGION="${4:-us-east}"
...
# Configuration
FIREWALL_LABEL="nov5-nest-multi-agents"
SSH_KEY_LABEL="nov5-multi-agent-key"
...
if [ ! -f "${SSH_KEY_LABEL}.pub" ]; then
    ...
    ssh-keygen -t rsa -b 4096 -f "$SSH_KEY_LABEL" -N "" -C "nov5-multi-agent-$DEPLOYMENT_ID"
fi
...
cd /home/ubuntu
sudo -u ubuntu git clone --single-branch --branch nov-5-demo https://github.com/DataWorksAI-com/NEST.git nanda-multi-agents-$DEPLOYMENT_ID
...
command=/home/ubuntu/nanda-multi-agents-$DEPLOYMENT_ID/env/bin/python nov-5-demo/nanda_agent.py
...
INSTANCE_ID=$(linode-cli linodes list --label "nov5-multi-agent-$DEPLOYMENT_ID" --text --no-headers --format="id" | head -n1)
if [ ! -n "$INSTANCE_ID" ]; then
    ...
    INSTANCE_ID=$(linode-cli linodes create \
        ...
        --region "$REGION" \
        --image "$IMAGE_ID" \
        --label "nov5-multi-agent-$DEPLOYMENT_ID" \
